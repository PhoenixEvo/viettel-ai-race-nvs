...
class Runner:
    ...
    def eval(self, step: int, stage: str = "val"):
        ...
        if world_rank == 0:
            ...
            psnr_norm = min(stats['psnr'] / 40.0, 1.0)
            comp_score = 0.4 * (1 - stats['lpips']) + 0.3 * stats['ssim'] + 0.3 * psnr_norm
            print(f"[CompScore] step={step} PSNR={stats['psnr']:.2f} SSIM={stats['ssim']:.4f} "
                  f"LPIPS={stats['lpips']:.4f} Score={comp_score:.4f}")
            # save stats as json
...
def save_image_competition(img_np: np.ndarray, out_path: Path):
    """Save image respecting competition filename convention (JPG quality=95)."""
...
def render_from_checkpoint(
    ...
    for i, test_pose in enumerate(test_poses):
        R = qvec_to_rotmat(test_pose.qvec)
        w2c = np.eye(4)
        w2c[:3, :3] = R
        ...
        save_image_competition(img_np, out_dir / test_pose.image_name)
        
        if (i + 1) % 10 == 0:
            print(f"Rendered {i + 1}/{len(test_poses)} images")
